...
def assess_risk(zip_code: str, advisory: Dict, zip_centroids: Dict) -> Dict:
	# Default LOW if ZIP unknown
	if zip_code not in zip_centroids:
		return {"risk": "LOW", "reason": "Unknown ZIP; defaulting to LOW."}
	...
	# Heuristics from brief:
	# - HIGH if inside radius (any category)
	# - HIGH if within +50 km at CAT2+
	# - MEDIUM if within +120 km buffer
	# - otherwise LOW
	if dist_km <= radius:
		risk = "HIGH"
